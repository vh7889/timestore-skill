---
name: timestore-rush
description: 使用统一配置文件执行 TimeStore 自动化任务：查询可买秒数、查询账户余额、查询充币地址、查询市场 KOL 列表、查询持仓列表、监控 issuerStatus、监控 KOL 发新帖、单次先查后买、达到目标金额后全仓卖出、组LP（先查最大可加秒数再计算金额并提交）、按阈值抢购、开盘后抢购、暴力抢购，并在关键结果后发送飞书通知。用户提到 TimeStore 抢购、查询 estimateVolume、查询 USDT 余额、查询充币地址、查询市场发行人/KOL、查询当前持仓、监控 issuerStatus、监控 KOL 发帖、单次直买、达到预期卖出全部、组LP、等待开盘、批量并发下单、飞书提醒时使用此技能。
---
...
```bash
python scripts/monitor_issuer_status.py --config config/config.toml
```

用途：

- 每 15 秒查询一次 `issuer/market` 中指定 `issuer_id` 的 `issuerStatus`。
- 只要 `issuerStatus != 1`，就发送飞书通知并退出。
- 这是从原抢购代码里的开盘检测逻辑拆出来的独立监控脚本，但判断条件改为 `!= 1`。
...
```bash
python scripts/monitor_kol_post.py --config config/config.toml
```

用途：

- 每 15 秒查询一次指定 `uid` 的时间线最新帖子。
- 只要最新帖不是你当前记录的那一条，就发送飞书通知并退出。
- 适合盯某个 KOL 是否发了新帖。

默认配置项（`config/config.toml`）：

- `post_monitor_uid`：要监控的 KOL 用户 `uid`
- `post_monitor_interval_seconds`：轮询间隔，默认 `15`
- `post_monitor_known_latest_id`：当前已知最新帖 `id`
- `post_monitor_known_latest_prefix`：当前已知最新帖开头文本

判断逻辑：

- 如果最新帖 `id != post_monitor_known_latest_id`，视为发了新帖
- 或者如果最新帖内容不再以 `post_monitor_known_latest_prefix` 开头，也视为发了新帖

可选参数：

- `--uid`：覆盖配置里的 `post_monitor_uid`
- `--interval`：覆盖配置里的轮询间隔
- `--known-id`：覆盖配置里的 `post_monitor_known_latest_id`
- `--known-prefix`：覆盖配置里的 `post_monitor_known_latest_prefix`
- `--once`：只检查一次
- `--raw`：打印完整接口返回
...
```bash
python scripts/query_max_buy.py --config config/config.toml
```

用途：

- 调用 `issuerOrder/maxBuy` 查询当前 `amount` 对应能买多少秒。
- 默认读取配置里的 `issuer_id` 和 `amount`。

可选参数：

- `--issuer-id`：覆盖配置里的 `issuer_id`
- `--amount`：覆盖配置里的 `amount`
- `--raw`：打印完整接口返回
