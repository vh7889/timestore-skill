---
name: timestore-rush
description: 使用统一配置文件执行 TimeStore 自动化任务：查询可买秒数、查询账户余额、查询充币地址、查询市场 KOL 列表、查询持仓列表、监控 issuerStatus、单次先查后买、达到目标金额后全仓卖出、组LP（先查最大可加秒数再计算金额并提交）、按阈值抢购、开盘后抢购、暴力抢购，并在关键结果后发送飞书通知。用户提到 TimeStore 抢购、查询 estimateVolume、查询 USDT 余额、查询充币地址、查询市场发行人/KOL、查询当前持仓、监控 issuerStatus、单次直买、达到预期卖出全部、组LP、等待开盘、批量并发下单、飞书提醒时使用此技能。
---
